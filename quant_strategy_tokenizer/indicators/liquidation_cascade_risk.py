"""
quant_strategy_tokenizer.indicators.liquidation_cascade_risk
==============================================================
Purpose: estimate cascade risk from liquidation stress as an atomic derivatives token.
Core idea: Blend liquidation pressure, OI stress, and price movement. Assumes large forced liquidations plus high leverage can raise continuation risk.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, LiquidationCascadeRiskParams, and
ModuleRunContext.
Outputs: LiquidationCascadeRiskReport with quality, last values, derivative direction, risk
...
INDICATOR = "liquidation_cascade_risk"


@dataclass
class LiquidationCascadeRiskParams(DerivativesParams):
    """Configuration for the liquidation_cascade_risk derivatives token.
...
@dataclass
class LiquidationCascadeRiskRequest:
    data: Any
    params: LiquidationCascadeRiskParams = field(default_factory=LiquidationCascadeRiskParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


LiquidationCascadeRiskReport = DerivativesReport


def normalize_input(request: LiquidationCascadeRiskRequest):
    return normalize_derivatives_input(request)


def run(request: LiquidationCascadeRiskRequest) -> ModuleResult[LiquidationCascadeRiskReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["LiquidationCascadeRiskParams", "LiquidationCascadeRiskRequest", "LiquidationCascadeRiskReport", "normalize_input", "run"]
