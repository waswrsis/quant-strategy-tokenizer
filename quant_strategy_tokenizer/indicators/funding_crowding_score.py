"""
quant_strategy_tokenizer.indicators.funding_crowding_score
============================================================
Purpose: combine funding extremes into a crowding score as an atomic derivatives token.
Core idea: Normalize funding z-score magnitude into a bounded pressure score. Assumes unusually expensive carry is a crowding warning rather than an entry signal by itself.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, FundingCrowdingScoreParams, and
ModuleRunContext.
Outputs: FundingCrowdingScoreReport with quality, last values, derivative direction, risk
...
INDICATOR = "funding_crowding_score"


@dataclass
class FundingCrowdingScoreParams(DerivativesParams):
    """Configuration for the funding_crowding_score derivatives token.
...
@dataclass
class FundingCrowdingScoreRequest:
    data: Any
    params: FundingCrowdingScoreParams = field(default_factory=FundingCrowdingScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FundingCrowdingScoreReport = DerivativesReport


def normalize_input(request: FundingCrowdingScoreRequest):
    return normalize_derivatives_input(request)


def run(request: FundingCrowdingScoreRequest) -> ModuleResult[FundingCrowdingScoreReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FundingCrowdingScoreParams", "FundingCrowdingScoreRequest", "FundingCrowdingScoreReport", "normalize_input", "run"]
