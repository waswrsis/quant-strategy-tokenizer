"""
quant_strategy_tokenizer.indicators.holder_age_trend
======================================================
Purpose: measure holder or UTXO age trend as an atomic on-chain token.
Core idea: Difference aggregate holder age over a short window. Assumes increasing age reflects holding behavior while decreasing age reflects coin movement.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, HolderAgeTrendParams, and
ModuleRunContext.
Outputs: HolderAgeTrendReport with quality, last values, network activity, flow,
...
INDICATOR = "holder_age_trend"


@dataclass
class HolderAgeTrendParams(OnChainParams):
    """Configuration for the holder_age_trend on-chain token.
...
@dataclass
class HolderAgeTrendRequest:
    data: Any
    params: HolderAgeTrendParams = field(default_factory=HolderAgeTrendParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


HolderAgeTrendReport = OnChainReport


def normalize_input(request: HolderAgeTrendRequest):
    return normalize_onchain_input(request)


def run(request: HolderAgeTrendRequest) -> ModuleResult[HolderAgeTrendReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["HolderAgeTrendParams", "HolderAgeTrendRequest", "HolderAgeTrendReport", "normalize_input", "run"]
