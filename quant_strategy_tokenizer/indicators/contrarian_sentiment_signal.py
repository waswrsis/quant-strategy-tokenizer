"""
quant_strategy_tokenizer.indicators.contrarian_sentiment_signal
=================================================================
Purpose: build a contrarian sentiment signal as an atomic sentiment token.
Core idea: Invert a consensus sentiment and attention composite. Assumes crowded optimism can be bearish and crowded panic can be bullish.
...
optional ExtractorSpec, ContrarianSentimentSignalParams, and ModuleRunContext.
Outputs: ContrarianSentimentSignalReport with quality, last values, sentiment direction/state,
...
INDICATOR = "contrarian_sentiment_signal"


@dataclass
class ContrarianSentimentSignalParams(SentimentParams):
    """Configuration for the contrarian_sentiment_signal sentiment token.
...
@dataclass
class ContrarianSentimentSignalRequest:
    data: Any
    params: ContrarianSentimentSignalParams = field(default_factory=ContrarianSentimentSignalParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ContrarianSentimentSignalReport = SentimentReport


def normalize_input(request: ContrarianSentimentSignalRequest):
    return normalize_sentiment_input(request)


def run(request: ContrarianSentimentSignalRequest) -> ModuleResult[ContrarianSentimentSignalReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ContrarianSentimentSignalParams", "ContrarianSentimentSignalRequest", "ContrarianSentimentSignalReport", "normalize_input", "run"]
