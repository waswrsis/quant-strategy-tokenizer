"""
quant_strategy_tokenizer.indicators.sopr_zscore
=================================================
Purpose: standardize SOPR against recent history as an atomic on-chain token.
Core idea: Compute a rolling z-score of SOPR. Assumes SOPR extremes can diagnose realized profit or capitulation regimes.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, SOPRZScoreParams, and
ModuleRunContext.
Outputs: SOPRZScoreReport with quality, last values, network activity, flow,
...
INDICATOR = "sopr_zscore"


@dataclass
class SOPRZScoreParams(OnChainParams):
    """Configuration for the sopr_zscore on-chain token.
...
@dataclass
class SOPRZScoreRequest:
    data: Any
    params: SOPRZScoreParams = field(default_factory=SOPRZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SOPRZScoreReport = OnChainReport


def normalize_input(request: SOPRZScoreRequest):
    return normalize_onchain_input(request)


def run(request: SOPRZScoreRequest) -> ModuleResult[SOPRZScoreReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SOPRZScoreParams", "SOPRZScoreRequest", "SOPRZScoreReport", "normalize_input", "run"]
