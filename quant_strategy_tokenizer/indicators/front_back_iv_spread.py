"""
quant_strategy_tokenizer.indicators.front_back_iv_spread
==========================================================
Purpose: measure front-month versus back-month IV spread as an atomic derivatives token.
Core idea: Subtract far-expiry IV from near-expiry IV. Assumes positive values can indicate front-end stress or event premium.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, FrontBackIVSpreadParams, and
ModuleRunContext.
Outputs: FrontBackIVSpreadReport with quality, last values, derivative direction, risk
...
INDICATOR = "front_back_iv_spread"


@dataclass
class FrontBackIVSpreadParams(DerivativesParams):
    """Configuration for the front_back_iv_spread derivatives token.
...
@dataclass
class FrontBackIVSpreadRequest:
    data: Any
    params: FrontBackIVSpreadParams = field(default_factory=FrontBackIVSpreadParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FrontBackIVSpreadReport = DerivativesReport


def normalize_input(request: FrontBackIVSpreadRequest):
    return normalize_derivatives_input(request)


def run(request: FrontBackIVSpreadRequest) -> ModuleResult[FrontBackIVSpreadReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FrontBackIVSpreadParams", "FrontBackIVSpreadRequest", "FrontBackIVSpreadReport", "normalize_input", "run"]
