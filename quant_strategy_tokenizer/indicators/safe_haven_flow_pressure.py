"""
quant_strategy_tokenizer.indicators.safe_haven_flow_pressure
==============================================================
Purpose: standardize safe-haven flow pressure as an atomic sentiment token.
Core idea: Compute a rolling z-score of safe-haven flows. Assumes flow into havens can proxy risk aversion.
...
optional ExtractorSpec, SafeHavenFlowPressureParams, and ModuleRunContext.
Outputs: SafeHavenFlowPressureReport with quality, last values, sentiment direction/state,
...
INDICATOR = "safe_haven_flow_pressure"


@dataclass
class SafeHavenFlowPressureParams(SentimentParams):
    """Configuration for the safe_haven_flow_pressure sentiment token.
...
@dataclass
class SafeHavenFlowPressureRequest:
    data: Any
    params: SafeHavenFlowPressureParams = field(default_factory=SafeHavenFlowPressureParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SafeHavenFlowPressureReport = SentimentReport


def normalize_input(request: SafeHavenFlowPressureRequest):
    return normalize_sentiment_input(request)


def run(request: SafeHavenFlowPressureRequest) -> ModuleResult[SafeHavenFlowPressureReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SafeHavenFlowPressureParams", "SafeHavenFlowPressureRequest", "SafeHavenFlowPressureReport", "normalize_input", "run"]
