"""
quant_strategy_tokenizer.indicators.mark_index_deviation
==========================================================
Purpose: measure mark-price deviation from index as an atomic derivatives token.
Core idea: Normalize mark minus index price by index price. Assumes sustained mark deviation is a liquidation and funding risk diagnostic.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, MarkIndexDeviationParams, and
ModuleRunContext.
Outputs: MarkIndexDeviationReport with quality, last values, derivative direction, risk
...
INDICATOR = "mark_index_deviation"


@dataclass
class MarkIndexDeviationParams(DerivativesParams):
    """Configuration for the mark_index_deviation derivatives token.
...
@dataclass
class MarkIndexDeviationRequest:
    data: Any
    params: MarkIndexDeviationParams = field(default_factory=MarkIndexDeviationParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


MarkIndexDeviationReport = DerivativesReport


def normalize_input(request: MarkIndexDeviationRequest):
    return normalize_derivatives_input(request)


def run(request: MarkIndexDeviationRequest) -> ModuleResult[MarkIndexDeviationReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["MarkIndexDeviationParams", "MarkIndexDeviationRequest", "MarkIndexDeviationReport", "normalize_input", "run"]
