"""
quant_strategy_tokenizer.indicators.percent_near_high
======================================================
Purpose: measure the percentage of instruments near rolling highs as an atomic breadth token.
Core idea: Count constituents within near_level_pct of their rolling high. Assumes broad proximity to highs shows constructive participation.
...
PercentNearHighParams, and ModuleRunContext.
Outputs: PercentNearHighReport with quality, last values, breadth direction/state,
...
INDICATOR = 'percent_near_high'


@dataclass
class PercentNearHighParams(BreadthParams):
    """Configuration for the percent_near_high breadth token.
...
@dataclass
class PercentNearHighRequest:
    data: Any
    params: PercentNearHighParams = field(default_factory=PercentNearHighParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PercentNearHighReport = BreadthReport


def normalize_input(request: PercentNearHighRequest):
    return normalize_breadth_input(request)


def run(request: PercentNearHighRequest) -> ModuleResult[PercentNearHighReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PercentNearHighParams", "PercentNearHighRequest", "PercentNearHighReport", "normalize_input", "run"]
