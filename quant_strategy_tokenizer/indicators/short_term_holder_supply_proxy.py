"""
quant_strategy_tokenizer.indicators.short_term_holder_supply_proxy
====================================================================
Purpose: proxy short-term holder supply as an atomic on-chain token.
Core idea: Sum supplied supply rows whose holder or UTXO age is below the configured short-term threshold. Assumes age buckets are supplied by the caller.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, ShortTermHolderSupplyProxyParams, and
ModuleRunContext.
Outputs: ShortTermHolderSupplyProxyReport with quality, last values, network activity, flow,
...
INDICATOR = "short_term_holder_supply_proxy"


@dataclass
class ShortTermHolderSupplyProxyParams(OnChainParams):
    """Configuration for the short_term_holder_supply_proxy on-chain token.
...
@dataclass
class ShortTermHolderSupplyProxyRequest:
    data: Any
    params: ShortTermHolderSupplyProxyParams = field(default_factory=ShortTermHolderSupplyProxyParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ShortTermHolderSupplyProxyReport = OnChainReport


def normalize_input(request: ShortTermHolderSupplyProxyRequest):
    return normalize_onchain_input(request)


def run(request: ShortTermHolderSupplyProxyRequest) -> ModuleResult[ShortTermHolderSupplyProxyReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ShortTermHolderSupplyProxyParams", "ShortTermHolderSupplyProxyRequest", "ShortTermHolderSupplyProxyReport", "normalize_input", "run"]
