"""
quant_strategy_tokenizer.indicators.wide_range
===============================================
Purpose: detect wide-range bars as an atomic structure token.
Core idea: Compare current high-low range with rolling average range. Assumes unusually large range signals expansion.
Inputs: raw caller-supplied market data, DataFrameSpec field mapping, optional
ExtractorSpec, WideRangeParams, and ModuleRunContext.
Outputs: WideRangeReport with quality, last values, structure bias/state, nearest
...
INDICATOR = 'wide_range'
INPUT_KIND = 'ohlc'


@dataclass
class WideRangeParams(StructureParams):
    """Configuration for the wide_range structure token.
...
@dataclass
class WideRangeRequest:
    data: Any
    params: WideRangeParams = field(default_factory=WideRangeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


WideRangeReport = StructureReport


def normalize_input(request: WideRangeRequest):
    return normalize_structure_input(request, INPUT_KIND)


def run(request: WideRangeRequest) -> ModuleResult[WideRangeReport]:
    return run_structure_indicator(INDICATOR, request, input_kind=INPUT_KIND, module_name=INDICATOR)


__all__ = ["WideRangeParams", "WideRangeRequest", "WideRangeReport", "normalize_input", "run"]
