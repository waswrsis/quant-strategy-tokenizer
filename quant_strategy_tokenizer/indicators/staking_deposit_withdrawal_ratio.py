"""
quant_strategy_tokenizer.indicators.staking_deposit_withdrawal_ratio
======================================================================
Purpose: compare staking deposits with withdrawals as an atomic on-chain token.
Core idea: Divide staking deposits by withdrawals. Assumes staking flows describe validator participation pressure for staking-based networks.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, StakingDepositWithdrawalRatioParams, and
ModuleRunContext.
Outputs: StakingDepositWithdrawalRatioReport with quality, last values, network activity, flow,
...
INDICATOR = "staking_deposit_withdrawal_ratio"


@dataclass
class StakingDepositWithdrawalRatioParams(OnChainParams):
    """Configuration for the staking_deposit_withdrawal_ratio on-chain token.
...
@dataclass
class StakingDepositWithdrawalRatioRequest:
    data: Any
    params: StakingDepositWithdrawalRatioParams = field(default_factory=StakingDepositWithdrawalRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


StakingDepositWithdrawalRatioReport = OnChainReport


def normalize_input(request: StakingDepositWithdrawalRatioRequest):
    return normalize_onchain_input(request)


def run(request: StakingDepositWithdrawalRatioRequest) -> ModuleResult[StakingDepositWithdrawalRatioReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["StakingDepositWithdrawalRatioParams", "StakingDepositWithdrawalRatioRequest", "StakingDepositWithdrawalRatioReport", "normalize_input", "run"]
