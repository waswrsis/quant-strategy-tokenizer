"""
quant_strategy_tokenizer.indicators.exchange_balance_change
=============================================================
Purpose: measure exchange reserve change as an atomic on-chain token.
Core idea: Calculate percentage change in exchange balance. Assumes exchange balances proxy liquid supply available on venues.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, ExchangeBalanceChangeParams, and
ModuleRunContext.
Outputs: ExchangeBalanceChangeReport with quality, last values, network activity, flow,
...
INDICATOR = "exchange_balance_change"


@dataclass
class ExchangeBalanceChangeParams(OnChainParams):
    """Configuration for the exchange_balance_change on-chain token.
...
@dataclass
class ExchangeBalanceChangeRequest:
    data: Any
    params: ExchangeBalanceChangeParams = field(default_factory=ExchangeBalanceChangeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ExchangeBalanceChangeReport = OnChainReport


def normalize_input(request: ExchangeBalanceChangeRequest):
    return normalize_onchain_input(request)


def run(request: ExchangeBalanceChangeRequest) -> ModuleResult[ExchangeBalanceChangeReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ExchangeBalanceChangeParams", "ExchangeBalanceChangeRequest", "ExchangeBalanceChangeReport", "normalize_input", "run"]
