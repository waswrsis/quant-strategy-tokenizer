"""
quant_strategy_tokenizer.indicators.funding_rate_zscore
=========================================================
Purpose: standardize funding rate pressure against recent history as an atomic derivatives token.
Core idea: Compute a rolling z-score of funding rates. Assumes extreme deviations from local history are more informative than the absolute funding level.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, FundingRateZScoreParams, and
ModuleRunContext.
Outputs: FundingRateZScoreReport with quality, last values, derivative direction, risk
...
@dataclass
class FundingRateZScoreParams(DerivativesParams):
    """Configuration for the funding_rate_zscore derivatives token.
...
@dataclass
class FundingRateZScoreRequest:
    data: Any
    params: FundingRateZScoreParams = field(default_factory=FundingRateZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FundingRateZScoreReport = DerivativesReport


def normalize_input(request: FundingRateZScoreRequest):
    return normalize_derivatives_input(request)


def run(request: FundingRateZScoreRequest) -> ModuleResult[FundingRateZScoreReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FundingRateZScoreParams", "FundingRateZScoreRequest", "FundingRateZScoreReport", "normalize_input", "run"]
