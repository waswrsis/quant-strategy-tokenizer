"""
quant_strategy_tokenizer.indicators.leverage_pressure_index
=============================================================
Purpose: combine futures leverage pressure inputs as an atomic derivatives token.
Core idea: Blend normalized funding, OI, basis, long/short, taker-flow, and liquidation pressures. Assumes several moderate warnings can matter more than one isolated metric.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, LeveragePressureIndexParams, and
ModuleRunContext.
Outputs: LeveragePressureIndexReport with quality, last values, derivative direction, risk
...
INDICATOR = "leverage_pressure_index"


@dataclass
class LeveragePressureIndexParams(DerivativesParams):
    """Configuration for the leverage_pressure_index derivatives token.
...
@dataclass
class LeveragePressureIndexRequest:
    data: Any
    params: LeveragePressureIndexParams = field(default_factory=LeveragePressureIndexParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


LeveragePressureIndexReport = DerivativesReport


def normalize_input(request: LeveragePressureIndexRequest):
    return normalize_derivatives_input(request)


def run(request: LeveragePressureIndexRequest) -> ModuleResult[LeveragePressureIndexReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["LeveragePressureIndexParams", "LeveragePressureIndexRequest", "LeveragePressureIndexReport", "normalize_input", "run"]
