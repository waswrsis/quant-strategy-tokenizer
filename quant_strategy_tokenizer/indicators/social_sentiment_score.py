"""
quant_strategy_tokenizer.indicators.social_sentiment_score
============================================================
Purpose: derive social polarity from mention counts as an atomic sentiment token.
Core idea: Compute positive-minus-negative mentions over total mentions. Assumes mention labels are supplied by the caller or vendor.
...
optional ExtractorSpec, SocialSentimentScoreParams, and ModuleRunContext.
Outputs: SocialSentimentScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "social_sentiment_score"


@dataclass
class SocialSentimentScoreParams(SentimentParams):
    """Configuration for the social_sentiment_score sentiment token.
...
@dataclass
class SocialSentimentScoreRequest:
    data: Any
    params: SocialSentimentScoreParams = field(default_factory=SocialSentimentScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SocialSentimentScoreReport = SentimentReport


def normalize_input(request: SocialSentimentScoreRequest):
    return normalize_sentiment_input(request)


def run(request: SocialSentimentScoreRequest) -> ModuleResult[SocialSentimentScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SocialSentimentScoreParams", "SocialSentimentScoreRequest", "SocialSentimentScoreReport", "normalize_input", "run"]
