"""
quant_strategy_tokenizer.indicators.address_growth_rate
=========================================================
Purpose: measure active-address growth as an atomic on-chain token.
Core idea: Calculate percentage change in active addresses over a short window. Assumes address growth is a rough adoption proxy, not a verified user count.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, AddressGrowthRateParams, and
ModuleRunContext.
Outputs: AddressGrowthRateReport with quality, last values, network activity, flow,
...
INDICATOR = "address_growth_rate"


@dataclass
class AddressGrowthRateParams(OnChainParams):
    """Configuration for the address_growth_rate on-chain token.
...
@dataclass
class AddressGrowthRateRequest:
    data: Any
    params: AddressGrowthRateParams = field(default_factory=AddressGrowthRateParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


AddressGrowthRateReport = OnChainReport


def normalize_input(request: AddressGrowthRateRequest):
    return normalize_onchain_input(request)


def run(request: AddressGrowthRateRequest) -> ModuleResult[AddressGrowthRateReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["AddressGrowthRateParams", "AddressGrowthRateRequest", "AddressGrowthRateReport", "normalize_input", "run"]
