"""
quant_strategy_tokenizer.indicators.put_call_volume_ratio
===========================================================
Purpose: measure put volume relative to call volume as an atomic derivatives token.
Core idea: Divide aggregate put volume by call volume. Assumes option volume by type proxies current hedging or speculative demand.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, PutCallVolumeRatioParams, and
ModuleRunContext.
Outputs: PutCallVolumeRatioReport with quality, last values, derivative direction, risk
...
INDICATOR = "put_call_volume_ratio"


@dataclass
class PutCallVolumeRatioParams(DerivativesParams):
    """Configuration for the put_call_volume_ratio derivatives token.
...
@dataclass
class PutCallVolumeRatioRequest:
    data: Any
    params: PutCallVolumeRatioParams = field(default_factory=PutCallVolumeRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PutCallVolumeRatioReport = DerivativesReport


def normalize_input(request: PutCallVolumeRatioRequest):
    return normalize_derivatives_input(request)


def run(request: PutCallVolumeRatioRequest) -> ModuleResult[PutCallVolumeRatioReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PutCallVolumeRatioParams", "PutCallVolumeRatioRequest", "PutCallVolumeRatioReport", "normalize_input", "run"]
