"""
quant_strategy_tokenizer.indicators.inside_bar
===============================================
Purpose: detect inside bars as an atomic structure token.
Core idea: Check whether current high/low are contained within the prior bar. Assumes inside bars represent short-term compression.
Inputs: raw caller-supplied market data, DataFrameSpec field mapping, optional
ExtractorSpec, InsideBarParams, and ModuleRunContext.
Outputs: InsideBarReport with quality, last values, structure bias/state, nearest
...
@dataclass
class InsideBarParams(StructureParams):
    """Configuration for the inside_bar structure token.
...
@dataclass
class InsideBarRequest:
    data: Any
    params: InsideBarParams = field(default_factory=InsideBarParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


InsideBarReport = StructureReport


def normalize_input(request: InsideBarRequest):
    return normalize_structure_input(request, INPUT_KIND)


def run(request: InsideBarRequest) -> ModuleResult[InsideBarReport]:
    return run_structure_indicator(INDICATOR, request, input_kind=INPUT_KIND, module_name=INDICATOR)


__all__ = ["InsideBarParams", "InsideBarRequest", "InsideBarReport", "normalize_input", "run"]
