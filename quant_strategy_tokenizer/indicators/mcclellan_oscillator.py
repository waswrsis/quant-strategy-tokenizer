"""
quant_strategy_tokenizer.indicators.mcclellan_oscillator
=========================================================
Purpose: calculate the McClellan oscillator from net advances as an atomic breadth token.
Core idea: Subtract slow EMA of net advances from fast EMA of net advances. Assumes breadth momentum is visible in the acceleration of advance/decline balance.
...
McClellanOscillatorParams, and ModuleRunContext.
Outputs: McClellanOscillatorReport with quality, last values, breadth direction/state,
...
INDICATOR = 'mcclellan_oscillator'


@dataclass
class McClellanOscillatorParams(BreadthParams):
    """Configuration for the mcclellan_oscillator breadth token.
...
@dataclass
class McClellanOscillatorRequest:
    data: Any
    params: McClellanOscillatorParams = field(default_factory=McClellanOscillatorParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


McClellanOscillatorReport = BreadthReport


def normalize_input(request: McClellanOscillatorRequest):
    return normalize_breadth_input(request)


def run(request: McClellanOscillatorRequest) -> ModuleResult[McClellanOscillatorReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["McClellanOscillatorParams", "McClellanOscillatorRequest", "McClellanOscillatorReport", "normalize_input", "run"]
