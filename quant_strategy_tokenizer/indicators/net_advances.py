"""
quant_strategy_tokenizer.indicators.net_advances
=================================================
Purpose: report advancing instruments minus declining instruments as an atomic breadth token.
Core idea: Subtract declines from advances. Assumes raw breadth imbalance is useful when universe size is stable or separately inspected.
...
NetAdvancesParams, and ModuleRunContext.
Outputs: NetAdvancesReport with quality, last values, breadth direction/state,
...
INDICATOR = 'net_advances'


@dataclass
class NetAdvancesParams(BreadthParams):
    """Configuration for the net_advances breadth token.
...
@dataclass
class NetAdvancesRequest:
    data: Any
    params: NetAdvancesParams = field(default_factory=NetAdvancesParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


NetAdvancesReport = BreadthReport


def normalize_input(request: NetAdvancesRequest):
    return normalize_breadth_input(request)


def run(request: NetAdvancesRequest) -> ModuleResult[NetAdvancesReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["NetAdvancesParams", "NetAdvancesRequest", "NetAdvancesReport", "normalize_input", "run"]
