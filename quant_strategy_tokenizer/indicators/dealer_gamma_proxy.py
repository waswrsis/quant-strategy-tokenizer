"""
quant_strategy_tokenizer.indicators.dealer_gamma_proxy
========================================================
Purpose: approximate dealer gamma pressure from chain greeks as an atomic derivatives token.
Core idea: Negate aggregate gamma exposure as a coarse dealer-side proxy. Assumes this is only a research proxy and not observed dealer inventory.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, DealerGammaProxyParams, and
ModuleRunContext.
Outputs: DealerGammaProxyReport with quality, last values, derivative direction, risk
...
INDICATOR = "dealer_gamma_proxy"


@dataclass
class DealerGammaProxyParams(DerivativesParams):
    """Configuration for the dealer_gamma_proxy derivatives token.
...
@dataclass
class DealerGammaProxyRequest:
    data: Any
    params: DealerGammaProxyParams = field(default_factory=DealerGammaProxyParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


DealerGammaProxyReport = DerivativesReport


def normalize_input(request: DealerGammaProxyRequest):
    return normalize_derivatives_input(request)


def run(request: DealerGammaProxyRequest) -> ModuleResult[DealerGammaProxyReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["DealerGammaProxyParams", "DealerGammaProxyRequest", "DealerGammaProxyReport", "normalize_input", "run"]
