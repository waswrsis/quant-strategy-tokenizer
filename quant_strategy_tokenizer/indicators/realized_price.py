"""
quant_strategy_tokenizer.indicators.realized_price
====================================================
Purpose: calculate or report realized price as an atomic on-chain token.
Core idea: Use supplied realized price or realized cap divided by supply. Assumes realized cap and supply are already vendor-normalized by the caller.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, RealizedPriceParams, and
ModuleRunContext.
Outputs: RealizedPriceReport with quality, last values, network activity, flow,
...
INDICATOR = "realized_price"


@dataclass
class RealizedPriceParams(OnChainParams):
    """Configuration for the realized_price on-chain token.
...
@dataclass
class RealizedPriceRequest:
    data: Any
    params: RealizedPriceParams = field(default_factory=RealizedPriceParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


RealizedPriceReport = OnChainReport


def normalize_input(request: RealizedPriceRequest):
    return normalize_onchain_input(request)


def run(request: RealizedPriceRequest) -> ModuleResult[RealizedPriceReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["RealizedPriceParams", "RealizedPriceRequest", "RealizedPriceReport", "normalize_input", "run"]
