"""
quant_strategy_tokenizer.indicators.funding_regime
====================================================
Purpose: classify the current funding environment as an atomic derivatives token.
Core idea: Score funding level and recent persistence into a regime label. Assumes persistent positive or negative funding can reveal crowded directional positioning.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, FundingRegimeParams, and
ModuleRunContext.
Outputs: FundingRegimeReport with quality, last values, derivative direction, risk
...
INDICATOR = "funding_regime"


@dataclass
class FundingRegimeParams(DerivativesParams):
    """Configuration for the funding_regime derivatives token.
...
@dataclass
class FundingRegimeRequest:
    data: Any
    params: FundingRegimeParams = field(default_factory=FundingRegimeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FundingRegimeReport = DerivativesReport


def normalize_input(request: FundingRegimeRequest):
    return normalize_derivatives_input(request)


def run(request: FundingRegimeRequest) -> ModuleResult[FundingRegimeReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FundingRegimeParams", "FundingRegimeRequest", "FundingRegimeReport", "normalize_input", "run"]
