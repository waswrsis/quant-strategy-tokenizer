"""
quant_strategy_tokenizer.indicators.miner_reserve_change
==========================================================
Purpose: measure miner reserve change as an atomic on-chain token.
Core idea: Calculate percentage change in miner reserves. Assumes miner balance labels are caller-supplied and meaningful for proof-of-work assets.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, MinerReserveChangeParams, and
ModuleRunContext.
Outputs: MinerReserveChangeReport with quality, last values, network activity, flow,
...
INDICATOR = "miner_reserve_change"


@dataclass
class MinerReserveChangeParams(OnChainParams):
    """Configuration for the miner_reserve_change on-chain token.
...
@dataclass
class MinerReserveChangeRequest:
    data: Any
    params: MinerReserveChangeParams = field(default_factory=MinerReserveChangeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


MinerReserveChangeReport = OnChainReport


def normalize_input(request: MinerReserveChangeRequest):
    return normalize_onchain_input(request)


def run(request: MinerReserveChangeRequest) -> ModuleResult[MinerReserveChangeReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["MinerReserveChangeParams", "MinerReserveChangeRequest", "MinerReserveChangeReport", "normalize_input", "run"]
