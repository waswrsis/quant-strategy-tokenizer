"""
quant_strategy_tokenizer.indicators.advance_decline_line
=========================================================
Purpose: accumulate net advances minus declines into a breadth line as an atomic breadth token.
Core idea: Cumulate advances minus declines through time. Assumes persistent net participation reveals broad market confirmation or deterioration.
...
AdvanceDeclineLineParams, and ModuleRunContext.
Outputs: AdvanceDeclineLineReport with quality, last values, breadth direction/state,
...
INDICATOR = 'advance_decline_line'


@dataclass
class AdvanceDeclineLineParams(BreadthParams):
    """Configuration for the advance_decline_line breadth token.
...
@dataclass
class AdvanceDeclineLineRequest:
    data: Any
    params: AdvanceDeclineLineParams = field(default_factory=AdvanceDeclineLineParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


AdvanceDeclineLineReport = BreadthReport


def normalize_input(request: AdvanceDeclineLineRequest):
    return normalize_breadth_input(request)


def run(request: AdvanceDeclineLineRequest) -> ModuleResult[AdvanceDeclineLineReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["AdvanceDeclineLineParams", "AdvanceDeclineLineRequest", "AdvanceDeclineLineReport", "normalize_input", "run"]
