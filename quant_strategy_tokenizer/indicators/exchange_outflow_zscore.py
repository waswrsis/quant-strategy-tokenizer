"""
quant_strategy_tokenizer.indicators.exchange_outflow_zscore
=============================================================
Purpose: standardize exchange outflow pressure as an atomic on-chain token.
Core idea: Compute a rolling z-score of exchange outflows. Assumes outflow spikes can proxy accumulation or custody migration.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, ExchangeOutflowZScoreParams, and
ModuleRunContext.
Outputs: ExchangeOutflowZScoreReport with quality, last values, network activity, flow,
...
INDICATOR = "exchange_outflow_zscore"


@dataclass
class ExchangeOutflowZScoreParams(OnChainParams):
    """Configuration for the exchange_outflow_zscore on-chain token.
...
@dataclass
class ExchangeOutflowZScoreRequest:
    data: Any
    params: ExchangeOutflowZScoreParams = field(default_factory=ExchangeOutflowZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ExchangeOutflowZScoreReport = OnChainReport


def normalize_input(request: ExchangeOutflowZScoreRequest):
    return normalize_onchain_input(request)


def run(request: ExchangeOutflowZScoreRequest) -> ModuleResult[ExchangeOutflowZScoreReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ExchangeOutflowZScoreParams", "ExchangeOutflowZScoreRequest", "ExchangeOutflowZScoreReport", "normalize_input", "run"]
