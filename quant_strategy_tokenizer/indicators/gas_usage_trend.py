"""
quant_strategy_tokenizer.indicators.gas_usage_trend
=====================================================
Purpose: measure gas usage growth as an atomic on-chain token.
Core idea: Calculate percentage change in gas used. Assumes gas usage captures contract or blockspace demand on compatible chains.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, GasUsageTrendParams, and
ModuleRunContext.
Outputs: GasUsageTrendReport with quality, last values, network activity, flow,
...
INDICATOR = "gas_usage_trend"


@dataclass
class GasUsageTrendParams(OnChainParams):
    """Configuration for the gas_usage_trend on-chain token.
...
@dataclass
class GasUsageTrendRequest:
    data: Any
    params: GasUsageTrendParams = field(default_factory=GasUsageTrendParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


GasUsageTrendReport = OnChainReport


def normalize_input(request: GasUsageTrendRequest):
    return normalize_onchain_input(request)


def run(request: GasUsageTrendRequest) -> ModuleResult[GasUsageTrendReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["GasUsageTrendParams", "GasUsageTrendRequest", "GasUsageTrendReport", "normalize_input", "run"]
