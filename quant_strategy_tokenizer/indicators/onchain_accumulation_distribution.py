"""
quant_strategy_tokenizer.indicators.onchain_accumulation_distribution
=======================================================================
Purpose: combine exchange flows with cohort balance divergence as an atomic on-chain token.
Core idea: Subtract exchange-flow pressure from whale versus retail balance momentum. Assumes withdrawals plus whale accumulation are supportive.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, OnchainAccumulationDistributionParams, and
ModuleRunContext.
Outputs: OnchainAccumulationDistributionReport with quality, last values, network activity, flow,
...
INDICATOR = "onchain_accumulation_distribution"


@dataclass
class OnchainAccumulationDistributionParams(OnChainParams):
    """Configuration for the onchain_accumulation_distribution on-chain token.
...
@dataclass
class OnchainAccumulationDistributionRequest:
    data: Any
    params: OnchainAccumulationDistributionParams = field(default_factory=OnchainAccumulationDistributionParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


OnchainAccumulationDistributionReport = OnChainReport


def normalize_input(request: OnchainAccumulationDistributionRequest):
    return normalize_onchain_input(request)


def run(request: OnchainAccumulationDistributionRequest) -> ModuleResult[OnchainAccumulationDistributionReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["OnchainAccumulationDistributionParams", "OnchainAccumulationDistributionRequest", "OnchainAccumulationDistributionReport", "normalize_input", "run"]
