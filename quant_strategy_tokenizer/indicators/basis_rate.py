"""
quant_strategy_tokenizer.indicators.basis_rate
================================================
Purpose: calculate futures/perp basis as a normalized spread as an atomic derivatives token.
Core idea: Use supplied basis when available, otherwise compare mark/futures price with spot or index. Assumes basis reflects carry, demand, and contract dislocation.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, BasisRateParams, and
ModuleRunContext.
Outputs: BasisRateReport with quality, last values, derivative direction, risk
...
INDICATOR = "basis_rate"


@dataclass
class BasisRateParams(DerivativesParams):
    """Configuration for the basis_rate derivatives token.
...
@dataclass
class BasisRateRequest:
    data: Any
    params: BasisRateParams = field(default_factory=BasisRateParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


BasisRateReport = DerivativesReport


def normalize_input(request: BasisRateRequest):
    return normalize_derivatives_input(request)


def run(request: BasisRateRequest) -> ModuleResult[BasisRateReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["BasisRateParams", "BasisRateRequest", "BasisRateReport", "normalize_input", "run"]
