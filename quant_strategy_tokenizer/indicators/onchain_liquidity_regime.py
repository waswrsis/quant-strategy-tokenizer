"""
quant_strategy_tokenizer.indicators.onchain_liquidity_regime
==============================================================
Purpose: classify stablecoin liquidity regime as an atomic on-chain token.
Core idea: Use stablecoin liquidity index as a regime input. Assumes stablecoin supply and exchange balances proxy available liquidity.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, OnchainLiquidityRegimeParams, and
ModuleRunContext.
Outputs: OnchainLiquidityRegimeReport with quality, last values, network activity, flow,
...
INDICATOR = "onchain_liquidity_regime"


@dataclass
class OnchainLiquidityRegimeParams(OnChainParams):
    """Configuration for the onchain_liquidity_regime on-chain token.
...
@dataclass
class OnchainLiquidityRegimeRequest:
    data: Any
    params: OnchainLiquidityRegimeParams = field(default_factory=OnchainLiquidityRegimeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


OnchainLiquidityRegimeReport = OnChainReport


def normalize_input(request: OnchainLiquidityRegimeRequest):
    return normalize_onchain_input(request)


def run(request: OnchainLiquidityRegimeRequest) -> ModuleResult[OnchainLiquidityRegimeReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["OnchainLiquidityRegimeParams", "OnchainLiquidityRegimeRequest", "OnchainLiquidityRegimeReport", "normalize_input", "run"]
