"""
quant_strategy_tokenizer.indicators.volatility_fear_proxy
===========================================================
Purpose: proxy fear pressure from volatility index as an atomic sentiment token.
Core idea: Map volatility-index z-score to a bounded pressure score. Assumes volatility can proxy fear but is not a direct sentiment survey.
...
optional ExtractorSpec, VolatilityFearProxyParams, and ModuleRunContext.
Outputs: VolatilityFearProxyReport with quality, last values, sentiment direction/state,
...
INDICATOR = "volatility_fear_proxy"


@dataclass
class VolatilityFearProxyParams(SentimentParams):
    """Configuration for the volatility_fear_proxy sentiment token.
...
@dataclass
class VolatilityFearProxyRequest:
    data: Any
    params: VolatilityFearProxyParams = field(default_factory=VolatilityFearProxyParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


VolatilityFearProxyReport = SentimentReport


def normalize_input(request: VolatilityFearProxyRequest):
    return normalize_sentiment_input(request)


def run(request: VolatilityFearProxyRequest) -> ModuleResult[VolatilityFearProxyReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["VolatilityFearProxyParams", "VolatilityFearProxyRequest", "VolatilityFearProxyReport", "normalize_input", "run"]
