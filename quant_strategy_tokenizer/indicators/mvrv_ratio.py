"""
quant_strategy_tokenizer.indicators.mvrv_ratio
================================================
Purpose: calculate market value to realized value as an atomic on-chain token.
Core idea: Use supplied MVRV or market cap divided by realized cap. Assumes realized cap approximates aggregate holder cost basis.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, MVRVRatioParams, and
ModuleRunContext.
Outputs: MVRVRatioReport with quality, last values, network activity, flow,
...
INDICATOR = "mvrv_ratio"


@dataclass
class MVRVRatioParams(OnChainParams):
    """Configuration for the mvrv_ratio on-chain token.
...
@dataclass
class MVRVRatioRequest:
    data: Any
    params: MVRVRatioParams = field(default_factory=MVRVRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


MVRVRatioReport = OnChainReport


def normalize_input(request: MVRVRatioRequest):
    return normalize_onchain_input(request)


def run(request: MVRVRatioRequest) -> ModuleResult[MVRVRatioReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["MVRVRatioParams", "MVRVRatioRequest", "MVRVRatioReport", "normalize_input", "run"]
