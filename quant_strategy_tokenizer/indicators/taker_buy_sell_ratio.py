"""
quant_strategy_tokenizer.indicators.taker_buy_sell_ratio
==========================================================
Purpose: report aggressive buy versus sell flow imbalance as an atomic derivatives token.
Core idea: Use the supplied taker buy/sell ratio directly. Assumes taker imbalance reflects short-horizon pressure from aggressive market orders.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, TakerBuySellRatioParams, and
ModuleRunContext.
Outputs: TakerBuySellRatioReport with quality, last values, derivative direction, risk
...
@dataclass
class TakerBuySellRatioParams(DerivativesParams):
    """Configuration for the taker_buy_sell_ratio derivatives token.
...
@dataclass
class TakerBuySellRatioRequest:
    data: Any
    params: TakerBuySellRatioParams = field(default_factory=TakerBuySellRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


TakerBuySellRatioReport = DerivativesReport


def normalize_input(request: TakerBuySellRatioRequest):
    return normalize_derivatives_input(request)


def run(request: TakerBuySellRatioRequest) -> ModuleResult[TakerBuySellRatioReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["TakerBuySellRatioParams", "TakerBuySellRatioRequest", "TakerBuySellRatioReport", "normalize_input", "run"]
