"""
quant_strategy_tokenizer.indicators.stablecoin_exchange_balance_change
========================================================================
Purpose: measure stablecoin exchange-balance change as an atomic on-chain token.
Core idea: Calculate percentage change in stablecoin balances on exchanges. Assumes exchange-held stablecoins proxy deployable crypto liquidity.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, StablecoinExchangeBalanceChangeParams, and
ModuleRunContext.
Outputs: StablecoinExchangeBalanceChangeReport with quality, last values, network activity, flow,
...
INDICATOR = "stablecoin_exchange_balance_change"


@dataclass
class StablecoinExchangeBalanceChangeParams(OnChainParams):
    """Configuration for the stablecoin_exchange_balance_change on-chain token.
...
@dataclass
class StablecoinExchangeBalanceChangeRequest:
    data: Any
    params: StablecoinExchangeBalanceChangeParams = field(default_factory=StablecoinExchangeBalanceChangeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


StablecoinExchangeBalanceChangeReport = OnChainReport


def normalize_input(request: StablecoinExchangeBalanceChangeRequest):
    return normalize_onchain_input(request)


def run(request: StablecoinExchangeBalanceChangeRequest) -> ModuleResult[StablecoinExchangeBalanceChangeReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["StablecoinExchangeBalanceChangeParams", "StablecoinExchangeBalanceChangeRequest", "StablecoinExchangeBalanceChangeReport", "normalize_input", "run"]
