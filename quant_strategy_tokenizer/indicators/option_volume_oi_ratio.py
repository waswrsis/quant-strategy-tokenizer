"""
quant_strategy_tokenizer.indicators.option_volume_oi_ratio
============================================================
Purpose: compare option trading activity to outstanding OI as an atomic derivatives token.
Core idea: Divide total option volume by total option open interest. Assumes high activity relative to OI flags unusual option turnover.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, OptionVolumeOIRatioParams, and
ModuleRunContext.
Outputs: OptionVolumeOIRatioReport with quality, last values, derivative direction, risk
...
INDICATOR = "option_volume_oi_ratio"


@dataclass
class OptionVolumeOIRatioParams(DerivativesParams):
    """Configuration for the option_volume_oi_ratio derivatives token.
...
@dataclass
class OptionVolumeOIRatioRequest:
    data: Any
    params: OptionVolumeOIRatioParams = field(default_factory=OptionVolumeOIRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


OptionVolumeOIRatioReport = DerivativesReport


def normalize_input(request: OptionVolumeOIRatioRequest):
    return normalize_derivatives_input(request)


def run(request: OptionVolumeOIRatioRequest) -> ModuleResult[OptionVolumeOIRatioReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["OptionVolumeOIRatioParams", "OptionVolumeOIRatioRequest", "OptionVolumeOIRatioReport", "normalize_input", "run"]
