"""
quant_strategy_tokenizer.indicators.oi_volume_ratio
=====================================================
Purpose: compare standing open interest to current turnover as an atomic derivatives token.
Core idea: Divide open interest by volume. Assumes high OI relative to turnover can imply crowded or slow-to-unwind positioning.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, OIVolumeRatioParams, and
ModuleRunContext.
Outputs: OIVolumeRatioReport with quality, last values, derivative direction, risk
...
INDICATOR = "oi_volume_ratio"


@dataclass
class OIVolumeRatioParams(DerivativesParams):
    """Configuration for the oi_volume_ratio derivatives token.
...
@dataclass
class OIVolumeRatioRequest:
    data: Any
    params: OIVolumeRatioParams = field(default_factory=OIVolumeRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


OIVolumeRatioReport = DerivativesReport


def normalize_input(request: OIVolumeRatioRequest):
    return normalize_derivatives_input(request)


def run(request: OIVolumeRatioRequest) -> ModuleResult[OIVolumeRatioReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["OIVolumeRatioParams", "OIVolumeRatioRequest", "OIVolumeRatioReport", "normalize_input", "run"]
