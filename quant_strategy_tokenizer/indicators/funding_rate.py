"""
quant_strategy_tokenizer.indicators.funding_rate
==================================================
Purpose: report the latest perpetual funding rate pressure as an atomic derivatives token.
Core idea: Use the supplied funding-rate time series directly. Assumes funding reflects the cost of holding perpetual exposure and can proxy long/short crowding.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, FundingRateParams, and
ModuleRunContext.
Outputs: FundingRateReport with quality, last values, derivative direction, risk
...
@dataclass
class FundingRateParams(DerivativesParams):
    """Configuration for the funding_rate derivatives token.
...
class FundingRateRequest:
    data: Any
    params: FundingRateParams = field(default_factory=FundingRateParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FundingRateReport = DerivativesReport


def normalize_input(request: FundingRateRequest):
    return normalize_derivatives_input(request)


def run(request: FundingRateRequest) -> ModuleResult[FundingRateReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FundingRateParams", "FundingRateRequest", "FundingRateReport", "normalize_input", "run"]
