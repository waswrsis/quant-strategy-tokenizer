"""
quant_strategy_tokenizer.indicators.fee_pressure
==================================================
Purpose: measure fees relative to transfer volume as an atomic on-chain token.
Core idea: Divide fees by transfer volume. Assumes fee burden relative to economic throughput can identify congestion pressure.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, FeePressureParams, and
ModuleRunContext.
Outputs: FeePressureReport with quality, last values, network activity, flow,
...
INDICATOR = "fee_pressure"


@dataclass
class FeePressureParams(OnChainParams):
    """Configuration for the fee_pressure on-chain token.
...
@dataclass
class FeePressureRequest:
    data: Any
    params: FeePressureParams = field(default_factory=FeePressureParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


FeePressureReport = OnChainReport


def normalize_input(request: FeePressureRequest):
    return normalize_onchain_input(request)


def run(request: FeePressureRequest) -> ModuleResult[FeePressureReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["FeePressureParams", "FeePressureRequest", "FeePressureReport", "normalize_input", "run"]
