"""
quant_strategy_tokenizer.indicators.risk_aversion_zscore
==========================================================
Purpose: standardize risk-aversion index as an atomic sentiment token.
Core idea: Compute a rolling z-score of risk aversion. Assumes risk-aversion feeds are source-specific and need local normalization.
...
optional ExtractorSpec, RiskAversionZScoreParams, and ModuleRunContext.
Outputs: RiskAversionZScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "risk_aversion_zscore"


@dataclass
class RiskAversionZScoreParams(SentimentParams):
    """Configuration for the risk_aversion_zscore sentiment token.
...
@dataclass
class RiskAversionZScoreRequest:
    data: Any
    params: RiskAversionZScoreParams = field(default_factory=RiskAversionZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


RiskAversionZScoreReport = SentimentReport


def normalize_input(request: RiskAversionZScoreRequest):
    return normalize_sentiment_input(request)


def run(request: RiskAversionZScoreRequest) -> ModuleResult[RiskAversionZScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["RiskAversionZScoreParams", "RiskAversionZScoreRequest", "RiskAversionZScoreReport", "normalize_input", "run"]
