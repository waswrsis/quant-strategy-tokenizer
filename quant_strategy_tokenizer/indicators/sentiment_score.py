"""
quant_strategy_tokenizer.indicators.sentiment_score
=====================================================
Purpose: report a caller-supplied or mention-derived sentiment score as an atomic sentiment token.
Core idea: Use supplied sentiment_score or derive positive-minus-negative mentions over total mentions. Assumes sentiment polarity is vendor-defined and not fetched by this module.
...
optional ExtractorSpec, SentimentScoreParams, and ModuleRunContext.
Outputs: SentimentScoreReport with quality, last values, sentiment direction/state,
...
@dataclass
class SentimentScoreParams(SentimentParams):
    """Configuration for the sentiment_score sentiment token.
...
@dataclass
class SentimentScoreRequest:
    data: Any
    params: SentimentScoreParams = field(default_factory=SentimentScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SentimentScoreReport = SentimentReport


def normalize_input(request: SentimentScoreRequest):
    return normalize_sentiment_input(request)


def run(request: SentimentScoreRequest) -> ModuleResult[SentimentScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SentimentScoreParams", "SentimentScoreRequest", "SentimentScoreReport", "normalize_input", "run"]
