"""
quant_strategy_tokenizer.indicators.long_term_holder_supply_proxy
===================================================================
Purpose: proxy long-term holder supply as an atomic on-chain token.
Core idea: Sum supplied supply rows whose holder or UTXO age exceeds the configured long-term threshold. Assumes age-bucket data is caller-provided and not inferred.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, LongTermHolderSupplyProxyParams, and
ModuleRunContext.
Outputs: LongTermHolderSupplyProxyReport with quality, last values, network activity, flow,
...
INDICATOR = "long_term_holder_supply_proxy"


@dataclass
class LongTermHolderSupplyProxyParams(OnChainParams):
    """Configuration for the long_term_holder_supply_proxy on-chain token.
...
@dataclass
class LongTermHolderSupplyProxyRequest:
    data: Any
    params: LongTermHolderSupplyProxyParams = field(default_factory=LongTermHolderSupplyProxyParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


LongTermHolderSupplyProxyReport = OnChainReport


def normalize_input(request: LongTermHolderSupplyProxyRequest):
    return normalize_onchain_input(request)


def run(request: LongTermHolderSupplyProxyRequest) -> ModuleResult[LongTermHolderSupplyProxyReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["LongTermHolderSupplyProxyParams", "LongTermHolderSupplyProxyRequest", "LongTermHolderSupplyProxyReport", "normalize_input", "run"]
