"""
quant_strategy_tokenizer.indicators.exchange_inflow_zscore
============================================================
Purpose: standardize exchange inflow pressure as an atomic on-chain token.
Core idea: Compute a rolling z-score of exchange inflows. Assumes inflow spikes are more meaningful relative to local history.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, ExchangeInflowZScoreParams, and
ModuleRunContext.
Outputs: ExchangeInflowZScoreReport with quality, last values, network activity, flow,
...
INDICATOR = "exchange_inflow_zscore"


@dataclass
class ExchangeInflowZScoreParams(OnChainParams):
    """Configuration for the exchange_inflow_zscore on-chain token.
...
@dataclass
class ExchangeInflowZScoreRequest:
    data: Any
    params: ExchangeInflowZScoreParams = field(default_factory=ExchangeInflowZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ExchangeInflowZScoreReport = OnChainReport


def normalize_input(request: ExchangeInflowZScoreRequest):
    return normalize_onchain_input(request)


def run(request: ExchangeInflowZScoreRequest) -> ModuleResult[ExchangeInflowZScoreReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ExchangeInflowZScoreParams", "ExchangeInflowZScoreRequest", "ExchangeInflowZScoreReport", "normalize_input", "run"]
