"""
quant_strategy_tokenizer.indicators.put_call_iv_skew
======================================================
Purpose: measure put versus call IV skew near ATM as an atomic derivatives token.
Core idea: Compare ATM put IV with ATM call IV. Assumes puts and calls with similar strike/expiry expose relative downside versus upside demand.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, PutCallIVSkewParams, and
ModuleRunContext.
Outputs: PutCallIVSkewReport with quality, last values, derivative direction, risk
...
INDICATOR = "put_call_iv_skew"


@dataclass
class PutCallIVSkewParams(DerivativesParams):
    """Configuration for the put_call_iv_skew derivatives token.
...
@dataclass
class PutCallIVSkewRequest:
    data: Any
    params: PutCallIVSkewParams = field(default_factory=PutCallIVSkewParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PutCallIVSkewReport = DerivativesReport


def normalize_input(request: PutCallIVSkewRequest):
    return normalize_derivatives_input(request)


def run(request: PutCallIVSkewRequest) -> ModuleResult[PutCallIVSkewReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PutCallIVSkewParams", "PutCallIVSkewRequest", "PutCallIVSkewReport", "normalize_input", "run"]
