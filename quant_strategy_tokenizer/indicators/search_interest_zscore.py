"""
quant_strategy_tokenizer.indicators.search_interest_zscore
============================================================
Purpose: standardize search interest as an atomic sentiment token.
Core idea: Compute a rolling z-score of supplied search interest. Assumes search data proxies attention but not directional intent.
...
optional ExtractorSpec, SearchInterestZScoreParams, and ModuleRunContext.
Outputs: SearchInterestZScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "search_interest_zscore"


@dataclass
class SearchInterestZScoreParams(SentimentParams):
    """Configuration for the search_interest_zscore sentiment token.
...
@dataclass
class SearchInterestZScoreRequest:
    data: Any
    params: SearchInterestZScoreParams = field(default_factory=SearchInterestZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SearchInterestZScoreReport = SentimentReport


def normalize_input(request: SearchInterestZScoreRequest):
    return normalize_sentiment_input(request)


def run(request: SearchInterestZScoreRequest) -> ModuleResult[SearchInterestZScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SearchInterestZScoreParams", "SearchInterestZScoreRequest", "SearchInterestZScoreReport", "normalize_input", "run"]
