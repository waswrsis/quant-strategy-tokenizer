"""
quant_strategy_tokenizer.indicators.social_volume
===================================================
Purpose: report social mention volume as an atomic sentiment token.
Core idea: Use supplied social volume directly. Assumes attention volume is meaningful only within the caller's source universe.
...
optional ExtractorSpec, SocialVolumeParams, and ModuleRunContext.
Outputs: SocialVolumeReport with quality, last values, sentiment direction/state,
...
@dataclass
class SocialVolumeParams(SentimentParams):
    """Configuration for the social_volume sentiment token.
...
@dataclass
class SocialVolumeRequest:
    data: Any
    params: SocialVolumeParams = field(default_factory=SocialVolumeParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SocialVolumeReport = SentimentReport


def normalize_input(request: SocialVolumeRequest):
    return normalize_sentiment_input(request)


def run(request: SocialVolumeRequest) -> ModuleResult[SocialVolumeReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SocialVolumeParams", "SocialVolumeRequest", "SocialVolumeReport", "normalize_input", "run"]
