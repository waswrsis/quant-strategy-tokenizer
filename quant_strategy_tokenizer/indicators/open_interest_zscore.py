"""
quant_strategy_tokenizer.indicators.open_interest_zscore
==========================================================
Purpose: standardize open-interest level versus history as an atomic derivatives token.
Core idea: Compute a rolling z-score of open interest. Assumes local OI extremes can indicate leverage crowding or capitulation.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, OpenInterestZScoreParams, and
ModuleRunContext.
Outputs: OpenInterestZScoreReport with quality, last values, derivative direction, risk
...
INDICATOR = "open_interest_zscore"


@dataclass
class OpenInterestZScoreParams(DerivativesParams):
    """Configuration for the open_interest_zscore derivatives token.
...
@dataclass
class OpenInterestZScoreRequest:
    data: Any
    params: OpenInterestZScoreParams = field(default_factory=OpenInterestZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


OpenInterestZScoreReport = DerivativesReport


def normalize_input(request: OpenInterestZScoreRequest):
    return normalize_derivatives_input(request)


def run(request: OpenInterestZScoreRequest) -> ModuleResult[OpenInterestZScoreReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["OpenInterestZScoreParams", "OpenInterestZScoreRequest", "OpenInterestZScoreReport", "normalize_input", "run"]
