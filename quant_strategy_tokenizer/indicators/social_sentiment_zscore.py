"""
quant_strategy_tokenizer.indicators.social_sentiment_zscore
=============================================================
Purpose: standardize social sentiment polarity as an atomic sentiment token.
Core idea: Compute a rolling z-score of mention-derived sentiment. Assumes local social polarity extremes are more informative than absolute label counts.
...
optional ExtractorSpec, SocialSentimentZScoreParams, and ModuleRunContext.
Outputs: SocialSentimentZScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "social_sentiment_zscore"


@dataclass
class SocialSentimentZScoreParams(SentimentParams):
    """Configuration for the social_sentiment_zscore sentiment token.
...
@dataclass
class SocialSentimentZScoreRequest:
    data: Any
    params: SocialSentimentZScoreParams = field(default_factory=SocialSentimentZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SocialSentimentZScoreReport = SentimentReport


def normalize_input(request: SocialSentimentZScoreRequest):
    return normalize_sentiment_input(request)


def run(request: SocialSentimentZScoreRequest) -> ModuleResult[SocialSentimentZScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SocialSentimentZScoreParams", "SocialSentimentZScoreRequest", "SocialSentimentZScoreReport", "normalize_input", "run"]
