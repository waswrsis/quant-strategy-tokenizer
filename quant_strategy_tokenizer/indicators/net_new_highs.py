"""
quant_strategy_tokenizer.indicators.net_new_highs
==================================================
Purpose: measure new highs minus new lows as an atomic breadth token.
Core idea: Subtract new lows from new highs. Assumes the balance of fresh strength versus weakness summarizes internal leadership.
...
NetNewHighsParams, and ModuleRunContext.
Outputs: NetNewHighsReport with quality, last values, breadth direction/state,
...
INDICATOR = 'net_new_highs'


@dataclass
class NetNewHighsParams(BreadthParams):
    """Configuration for the net_new_highs breadth token.
...
@dataclass
class NetNewHighsRequest:
    data: Any
    params: NetNewHighsParams = field(default_factory=NetNewHighsParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


NetNewHighsReport = BreadthReport


def normalize_input(request: NetNewHighsRequest):
    return normalize_breadth_input(request)


def run(request: NetNewHighsRequest) -> ModuleResult[NetNewHighsReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["NetNewHighsParams", "NetNewHighsRequest", "NetNewHighsReport", "normalize_input", "run"]
