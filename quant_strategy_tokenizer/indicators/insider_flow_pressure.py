"""
quant_strategy_tokenizer.indicators.insider_flow_pressure
===========================================================
Purpose: standardize insider net buying as an atomic sentiment token.
Core idea: Compute a z-score of insider buy value minus sell value. Assumes insider net flow can proxy informed sentiment.
...
optional ExtractorSpec, InsiderFlowPressureParams, and ModuleRunContext.
Outputs: InsiderFlowPressureReport with quality, last values, sentiment direction/state,
...
INDICATOR = "insider_flow_pressure"


@dataclass
class InsiderFlowPressureParams(SentimentParams):
    """Configuration for the insider_flow_pressure sentiment token.
...
@dataclass
class InsiderFlowPressureRequest:
    data: Any
    params: InsiderFlowPressureParams = field(default_factory=InsiderFlowPressureParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


InsiderFlowPressureReport = SentimentReport


def normalize_input(request: InsiderFlowPressureRequest):
    return normalize_sentiment_input(request)


def run(request: InsiderFlowPressureRequest) -> ModuleResult[InsiderFlowPressureReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["InsiderFlowPressureParams", "InsiderFlowPressureRequest", "InsiderFlowPressureReport", "normalize_input", "run"]
