"""
quant_strategy_tokenizer.indicators.sopr
==========================================
Purpose: report spent output profit ratio as an atomic on-chain token.
Core idea: Use the supplied SOPR series directly. Assumes the caller provides a consistent SOPR definition from their data source.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, SOPRParams, and
ModuleRunContext.
Outputs: SOPRReport with quality, last values, network activity, flow,
...
@dataclass
class SOPRParams(OnChainParams):
    """Configuration for the sopr on-chain token.
...
class SOPRRequest:
    data: Any
    params: SOPRParams = field(default_factory=SOPRParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


SOPRReport = OnChainReport


def normalize_input(request: SOPRRequest):
    return normalize_onchain_input(request)


def run(request: SOPRRequest) -> ModuleResult[SOPRReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["SOPRParams", "SOPRRequest", "SOPRReport", "normalize_input", "run"]
