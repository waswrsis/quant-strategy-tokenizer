"""
quant_strategy_tokenizer.indicators.perp_spot_deviation
=========================================================
Purpose: measure perpetual or mark deviation from spot as an atomic derivatives token.
Core idea: Compare mark/perp price with spot price. Assumes spot-perp divergence can indicate leverage demand or exchange-specific dislocation.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, PerpSpotDeviationParams, and
ModuleRunContext.
Outputs: PerpSpotDeviationReport with quality, last values, derivative direction, risk
...
INDICATOR = "perp_spot_deviation"


@dataclass
class PerpSpotDeviationParams(DerivativesParams):
    """Configuration for the perp_spot_deviation derivatives token.
...
@dataclass
class PerpSpotDeviationRequest:
    data: Any
    params: PerpSpotDeviationParams = field(default_factory=PerpSpotDeviationParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PerpSpotDeviationReport = DerivativesReport


def normalize_input(request: PerpSpotDeviationRequest):
    return normalize_derivatives_input(request)


def run(request: PerpSpotDeviationRequest) -> ModuleResult[PerpSpotDeviationReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PerpSpotDeviationParams", "PerpSpotDeviationRequest", "PerpSpotDeviationReport", "normalize_input", "run"]
