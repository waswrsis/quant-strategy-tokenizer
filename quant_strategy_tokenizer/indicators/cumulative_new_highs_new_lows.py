"""
quant_strategy_tokenizer.indicators.cumulative_new_highs_new_lows
==================================================================
Purpose: accumulate net new highs through time as an atomic breadth token.
Core idea: Cumulate new highs minus new lows. Assumes persistent leadership breadth is visible in the cumulative high-low balance.
...
CumulativeNewHighsNewLowsParams, and ModuleRunContext.
Outputs: CumulativeNewHighsNewLowsReport with quality, last values, breadth direction/state,
...
INDICATOR = 'cumulative_new_highs_new_lows'


@dataclass
class CumulativeNewHighsNewLowsParams(BreadthParams):
    """Configuration for the cumulative_new_highs_new_lows breadth token.
...
@dataclass
class CumulativeNewHighsNewLowsRequest:
    data: Any
    params: CumulativeNewHighsNewLowsParams = field(default_factory=CumulativeNewHighsNewLowsParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


CumulativeNewHighsNewLowsReport = BreadthReport


def normalize_input(request: CumulativeNewHighsNewLowsRequest):
    return normalize_breadth_input(request)


def run(request: CumulativeNewHighsNewLowsRequest) -> ModuleResult[CumulativeNewHighsNewLowsReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["CumulativeNewHighsNewLowsParams", "CumulativeNewHighsNewLowsRequest", "CumulativeNewHighsNewLowsReport", "normalize_input", "run"]
