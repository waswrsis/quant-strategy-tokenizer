"""
quant_strategy_tokenizer.indicators.percent_near_low
=====================================================
Purpose: measure the percentage of instruments near rolling lows as an atomic breadth token.
Core idea: Count constituents within near_level_pct of their rolling low. Assumes broad proximity to lows shows internal stress.
...
PercentNearLowParams, and ModuleRunContext.
Outputs: PercentNearLowReport with quality, last values, breadth direction/state,
...
INDICATOR = 'percent_near_low'


@dataclass
class PercentNearLowParams(BreadthParams):
    """Configuration for the percent_near_low breadth token.
...
@dataclass
class PercentNearLowRequest:
    data: Any
    params: PercentNearLowParams = field(default_factory=PercentNearLowParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PercentNearLowReport = BreadthReport


def normalize_input(request: PercentNearLowRequest):
    return normalize_breadth_input(request)


def run(request: PercentNearLowRequest) -> ModuleResult[PercentNearLowReport]:
    return run_breadth_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PercentNearLowParams", "PercentNearLowRequest", "PercentNearLowReport", "normalize_input", "run"]
