"""
quant_strategy_tokenizer.indicators.short_interest_ratio
==========================================================
Purpose: report short interest ratio as an atomic sentiment token.
Core idea: Use supplied short interest directly. Assumes the input is a ratio or comparable short-interest measure.
...
optional ExtractorSpec, ShortInterestRatioParams, and ModuleRunContext.
Outputs: ShortInterestRatioReport with quality, last values, sentiment direction/state,
...
INDICATOR = "short_interest_ratio"


@dataclass
class ShortInterestRatioParams(SentimentParams):
    """Configuration for the short_interest_ratio sentiment token.
...
@dataclass
class ShortInterestRatioRequest:
    data: Any
    params: ShortInterestRatioParams = field(default_factory=ShortInterestRatioParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ShortInterestRatioReport = SentimentReport


def normalize_input(request: ShortInterestRatioRequest):
    return normalize_sentiment_input(request)


def run(request: ShortInterestRatioRequest) -> ModuleResult[ShortInterestRatioReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ShortInterestRatioParams", "ShortInterestRatioRequest", "ShortInterestRatioReport", "normalize_input", "run"]
