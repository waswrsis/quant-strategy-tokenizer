"""
quant_strategy_tokenizer.indicators.exchange_flow_pressure
============================================================
Purpose: standardize net exchange flow pressure as an atomic on-chain token.
Core idea: Compute a z-score of exchange netflow. Assumes positive netflow is distribution pressure and negative netflow is accumulation pressure.
Inputs: caller-supplied on-chain rows, age-bucket rows, account/token rows,
DataFrameSpec field mapping, optional ExtractorSpec, ExchangeFlowPressureParams, and
ModuleRunContext.
Outputs: ExchangeFlowPressureReport with quality, last values, network activity, flow,
...
INDICATOR = "exchange_flow_pressure"


@dataclass
class ExchangeFlowPressureParams(OnChainParams):
    """Configuration for the exchange_flow_pressure on-chain token.
...
@dataclass
class ExchangeFlowPressureRequest:
    data: Any
    params: ExchangeFlowPressureParams = field(default_factory=ExchangeFlowPressureParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


ExchangeFlowPressureReport = OnChainReport


def normalize_input(request: ExchangeFlowPressureRequest):
    return normalize_onchain_input(request)


def run(request: ExchangeFlowPressureRequest) -> ModuleResult[ExchangeFlowPressureReport]:
    return run_onchain_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["ExchangeFlowPressureParams", "ExchangeFlowPressureRequest", "ExchangeFlowPressureReport", "normalize_input", "run"]
