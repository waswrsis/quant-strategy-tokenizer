"""
quant_strategy_tokenizer.indicators.news_sentiment_score
==========================================================
Purpose: report news sentiment score as an atomic sentiment token.
Core idea: Use the supplied news sentiment series directly. Assumes the caller controls article selection and scoring methodology.
...
optional ExtractorSpec, NewsSentimentScoreParams, and ModuleRunContext.
Outputs: NewsSentimentScoreReport with quality, last values, sentiment direction/state,
...
@dataclass
class NewsSentimentScoreParams(SentimentParams):
    """Configuration for the news_sentiment_score sentiment token.
...
@dataclass
class NewsSentimentScoreRequest:
    data: Any
    params: NewsSentimentScoreParams = field(default_factory=NewsSentimentScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


NewsSentimentScoreReport = SentimentReport


def normalize_input(request: NewsSentimentScoreRequest):
    return normalize_sentiment_input(request)


def run(request: NewsSentimentScoreRequest) -> ModuleResult[NewsSentimentScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["NewsSentimentScoreParams", "NewsSentimentScoreRequest", "NewsSentimentScoreReport", "normalize_input", "run"]
