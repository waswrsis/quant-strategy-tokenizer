"""
quant_strategy_tokenizer.indicators.rating_score
==================================================
Purpose: report analyst rating score as an atomic sentiment token.
Core idea: Use supplied rating score directly. Assumes the caller maps vendor ratings onto a consistent numeric scale.
...
optional ExtractorSpec, RatingScoreParams, and ModuleRunContext.
Outputs: RatingScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "rating_score"


@dataclass
class RatingScoreParams(SentimentParams):
    """Configuration for the rating_score sentiment token.
...
@dataclass
class RatingScoreRequest:
    data: Any
    params: RatingScoreParams = field(default_factory=RatingScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


RatingScoreReport = SentimentReport


def normalize_input(request: RatingScoreRequest):
    return normalize_sentiment_input(request)


def run(request: RatingScoreRequest) -> ModuleResult[RatingScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["RatingScoreParams", "RatingScoreRequest", "RatingScoreReport", "normalize_input", "run"]
