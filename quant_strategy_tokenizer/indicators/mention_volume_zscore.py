"""
quant_strategy_tokenizer.indicators.mention_volume_zscore
===========================================================
Purpose: standardize total mention attention as an atomic sentiment token.
Core idea: Compute a rolling z-score of total mentions. Assumes attention spikes are source-relative events.
...
optional ExtractorSpec, MentionVolumeZScoreParams, and ModuleRunContext.
Outputs: MentionVolumeZScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "mention_volume_zscore"


@dataclass
class MentionVolumeZScoreParams(SentimentParams):
    """Configuration for the mention_volume_zscore sentiment token.
...
@dataclass
class MentionVolumeZScoreRequest:
    data: Any
    params: MentionVolumeZScoreParams = field(default_factory=MentionVolumeZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


MentionVolumeZScoreReport = SentimentReport


def normalize_input(request: MentionVolumeZScoreRequest):
    return normalize_sentiment_input(request)


def run(request: MentionVolumeZScoreRequest) -> ModuleResult[MentionVolumeZScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["MentionVolumeZScoreParams", "MentionVolumeZScoreRequest", "MentionVolumeZScoreReport", "normalize_input", "run"]
