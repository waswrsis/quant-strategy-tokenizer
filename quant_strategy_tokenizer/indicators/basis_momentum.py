"""
quant_strategy_tokenizer.indicators.basis_momentum
====================================================
Purpose: measure basis expansion or compression as an atomic derivatives token.
Core idea: Compare fast and slow basis averages. Assumes accelerating basis can signal changing carry pressure or crowded positioning.
Inputs: caller-supplied futures/perpetual rows, option-chain rows, aggregate
rows, DataFrameSpec field mapping, optional ExtractorSpec, BasisMomentumParams, and
ModuleRunContext.
Outputs: BasisMomentumReport with quality, last values, derivative direction, risk
...
INDICATOR = "basis_momentum"


@dataclass
class BasisMomentumParams(DerivativesParams):
    """Configuration for the basis_momentum derivatives token.
...
@dataclass
class BasisMomentumRequest:
    data: Any
    params: BasisMomentumParams = field(default_factory=BasisMomentumParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


BasisMomentumReport = DerivativesReport


def normalize_input(request: BasisMomentumRequest):
    return normalize_derivatives_input(request)


def run(request: BasisMomentumRequest) -> ModuleResult[BasisMomentumReport]:
    return run_derivatives_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["BasisMomentumParams", "BasisMomentumRequest", "BasisMomentumReport", "normalize_input", "run"]
