"""
quant_strategy_tokenizer.indicators.policy_uncertainty_zscore
===============================================================
Purpose: standardize policy uncertainty as an atomic sentiment token.
Core idea: Compute a rolling z-score of policy uncertainty. Assumes high uncertainty can pressure risk appetite.
...
optional ExtractorSpec, PolicyUncertaintyZScoreParams, and ModuleRunContext.
Outputs: PolicyUncertaintyZScoreReport with quality, last values, sentiment direction/state,
...
INDICATOR = "policy_uncertainty_zscore"


@dataclass
class PolicyUncertaintyZScoreParams(SentimentParams):
    """Configuration for the policy_uncertainty_zscore sentiment token.
...
@dataclass
class PolicyUncertaintyZScoreRequest:
    data: Any
    params: PolicyUncertaintyZScoreParams = field(default_factory=PolicyUncertaintyZScoreParams)
    spec: DataFrameSpec = field(default_factory=DataFrameSpec)
    extractor: Optional[ExtractorSpec] = None
    context: ModuleRunContext = field(default_factory=lambda: ModuleRunContext(module=INDICATOR))


PolicyUncertaintyZScoreReport = SentimentReport


def normalize_input(request: PolicyUncertaintyZScoreRequest):
    return normalize_sentiment_input(request)


def run(request: PolicyUncertaintyZScoreRequest) -> ModuleResult[PolicyUncertaintyZScoreReport]:
    return run_sentiment_indicator(INDICATOR, request, module_name=INDICATOR)


__all__ = ["PolicyUncertaintyZScoreParams", "PolicyUncertaintyZScoreRequest", "PolicyUncertaintyZScoreReport", "normalize_input", "run"]
