"""TokenPack qstpkg manifest policy helpers for Token System v2."""
...
from quant_strategy_tokenizer.tokens_v2.pack import EmbeddedTokenPolicy, TokenPackManifestV2
...
class TokenPackPackageEntryV04(BaseModel):
    """One TokenPack reference recorded in a qstpkg manifest."""
...
class TokenPacksPackageSectionV04(BaseModel):
    """Optional TokenPack propagation section for qstpkg manifests."""
...
def token_pack_package_entry_from_pack(
    ...
    *,
    ...
    """Build one deterministic qstpkg TokenPack manifest entry."""
...
def token_pack_package_section_from_packs(
    ...
    *,
    ...
    """Build the qstpkg TokenPack section from pack metadata."""
...
def verify_token_pack_package_section(
    ...
    """Verify qstpkg TokenPack references without executing embedded source."""
        ...
        if actual_hash is None:
            diagnostics.append(
                _diagnostic(
                    ...
                    (
                        f"TokenPack {entry.pack_id} {entry.version} referenced by qstpkg "
                        "is not available."
                    ),
                )
            )
