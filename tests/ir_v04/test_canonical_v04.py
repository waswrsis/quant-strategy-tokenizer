...
def test_canonicalization_does_not_call_legacy_canonicalizer(
    monkeypatch,
) -> None:
    import quant_strategy_tokenizer.ir.canonicalize as legacy_canonicalize

    def fail(*args: object, **kwargs: object) -> None:
        raise AssertionError("legacy canonicalizer should not be called")

    monkeypatch.setattr(legacy_canonicalize, "canonicalize", fail)

    canonicalize_v04(_strategy())
