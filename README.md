...
WP9 is accepted as the Custom Token Runtime + PV-D stage. It separates integrity verification from profile/approval authorization, requires `allow_token=true` and `ack_risk=true` for executable approvals, enforces run-id/expiry-bound ExecutionGrants, verifies qstpkg TokenPack metadata without executing code, and adds the deterministic custom Kalman PV-D reference case. It does not add a sandbox, WP10 migration, broad v0.4 runtime execution, or portable qstpkg trust.
