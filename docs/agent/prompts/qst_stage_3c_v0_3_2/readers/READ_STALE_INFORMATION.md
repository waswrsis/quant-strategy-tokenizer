...
Use this reader when the task depends on current repository evidence about stale names, stale suffixes, and stale current-state claims.
The reader gathers facts; it does not decide or edit by itself.

## Read

- Active code, docs, tests, schemas, examples, or CI files that define stale names, stale suffixes, and stale current-state claims.
