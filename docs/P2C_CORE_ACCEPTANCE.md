# P2c-core Acceptance Record

Date: 2026-05-14
Implementation commit: `4aeacf974cf1d4090a380a771484e9b8ae32f8ad`
...
- Merkle node fingerprints.
- Execution Plan CSE.
- Runtime cache reuse.
- Trace cache-hit metadata.
- `qst fingerprint` debug command.

Deferred scope:

- P2a-2 recipe generator.
...
- `python -m quant_strategy_tokenizer.cli fingerprint strategies/uses_cse_duplicate_chain.qst.yaml`: PASS
- `python -m quant_strategy_tokenizer.cli execute strategies/uses_cse_duplicate_chain.qst.yaml --market examples/sample_market_btc_15m.csv --trace-path "$env:TEMP/qst_cse_trace.json"`: PASS
- `python -m quant_strategy_tokenizer.cli explain-trace "$env:TEMP/qst_cse_trace.json" --level raw`: PASS
...
- `python -m pytest --cov=quant_strategy_tokenizer --cov-fail-under=80`: PASS, 186 tests, 87.15% coverage

## CI

GitHub Actions run: https://github.com/waswrsis/Quant-Strategy-Tokenizer/actions/runs/25863752446
Result: PASS
...
- canonical graph nodes remain: 5
- `n1` cache-hit reuses `n0`
- `n3` cache-hit reuses `n2`
- fingerprints use `fp_sha256:<64 hex>`
- cache metadata is written only to trace/debug output, not to canonical Strategy Content IR or P0/P1 hash material

## Scope Confirmation

P2c-core CSE is implemented only in the Execution Plan layer. No canonicalization CSE, recipe generator, advanced mutation, kernel substitution, new primitive token, or new recipe was added.
